...
| Code | Raised by | Meaning / typical cause | Surface | What to do |
| --- | --- | --- | --- | --- |
| `OL_E_INSTALLATION_BUSY` | `InstallationLease.Acquire`; `OmsiLaunchService.RecoverPendingAsync`; `FileConfigurationTransaction.RestorePendingAsync` | The installation lease (`Local\OmsiLaunch.Installation.<hash>`) is held by another owner in this logon session, or a journaled OMSI process (PID + creation time + executable path; or any `Omsi.exe` from the root for a journal past `HandoffCreated` without a PID) is still alive. | Start: session diagnostic via `OL_E_START_SESSION`. Recovery: thrown (`InvalidOperationException` / `IOException`). CLI exit 7. | Stop the other owner (`session stop`) or wait for OMSI to exit, then retry or `/recover`. |
...
| `OL_E_UNKNOWN_SETTING` | `SessionPlanner`; `CliInput.BuildSpecAsync`; `BuildTransactionalOverlays` | The key is not in `ConfigurationCatalog`. | Plan diagnostic; CLI exit 2 | Use a catalog key. |

## LaunchSpec

| Code | Raised by | Meaning / typical cause | Surface | What to do |
| --- | --- | --- | --- | --- |
| `OL_E_SPEC_INVALID` | `LaunchSpecJson.Parse` | The root is not a JSON object, or deserialization produced no record. | Thrown (`InvalidDataException`), CLI exit 2 | Fix the file ([LaunchSpec](launchspec.md)). |
...
| `OL_E_CONTROL_HANDLER_FAILED` | `LocalControlPlane.ServeAsync` | The owner's handler threw an exception whose message carries no `OL_E_` code (for example the session was already closed), or the handler's reply could not be serialized. | Control reply | Read `session status`; restart the owner if it is gone. |
...
| `OL_E_CONTROL_MESSAGE_TOO_LARGE` | `LocalControlPlane.TryRequestAsync` (client) | The client's own serialized request exceeds 64 KiB. Reported to the caller; nothing is sent. | Control reply / CLI envelope | Reduce the request. |
...
| `OL_E_PROCESS_EXITED_EARLY` | `OmsiLaunchService.SuperviseAsync` | OMSI exited before `gameplay.entered` (crash, an OMSI error dialog was closed, the window was closed). | Session diagnostic (`Failed`); restore runs | Check OMSI's own logs and `logfile.txt`; look at `RuntimeEvents` for the last plugin event. |
...
| `OL_E_RUNTIME_BASELINE_UNAVAILABLE` | `RuntimeBatch` (`/runtime-write-batch`, INTERNAL harness) | The baseline `time.read`/`camera.read` failed, so the write test was skipped. | Batch artifact only | Not user-facing. |
...
| `OL_E_RUNTIME_REQUEST_TIMEOUT` | `CurrentRuntimeCommandStore.RequestAsync` | No response within `timeout`; the slot is reset and a late response is discarded. | Thrown (`TimeoutException`); CLI exit 5 | Retry with a longer timeout; check that OMSI is not blocked (modal dialog, loading). |
...
| `OL_E_RUNTIME_VARIABLE_UNAVAILABLE` | `OmsiRuntimeReaders` | The variable slot or value address is null. | Runtime detail | The variable is not materialized for this vehicle. |
...
| `OL_E_SESSION_START_FAILED` | `WindowsHost.ShowFailure` (OmsiLaunchW dialog) | Fallback code shown when a launch plan is not runnable or the session did not reach gameplay, and no `OL_E_` diagnostic exists. | Message box only | Read `.omsilaunch\diagnostics`. |
...
| `OL_E_SESSION_PROFILE_SETTING_UNKNOWN` | A preset setting key is not in the catalog. | Use a catalog key. |
