...
Every standalone owner session (started with `OmsiLaunch.exe` or `OmsiLaunchW.exe`) shows a notification-area icon that reports the session and lets the user end it. This page specifies the indicator as implemented by `SessionTrayIndicator`, `StatusWindow` and `StopConfirmationWindow` in `tools\OmsiLaunch.Cli\WindowsHost.cs`, the read-only presenter `SessionStatusPresenter` (`tools\OmsiLaunch.Cli\SessionStatusPresenter.cs`) and the string registry `WindowsUiStrings` (`tools\OmsiLaunch.Cli\WindowsUiStrings.cs`), together with the `OmsiLaunchW.exe` failure dialogs of `WindowsHost`. The tray is a presentation adapter only: it owns neither OMSI nor recovery, and its stop action signals the same canonical owner stop path as `session stop` (see [CLI reference](cli.md) and [local control](local-control.md)).
...
1. `StopConfirmationWindow`: title `End session?`, message `OMSI 2 will be closed and the OmsiLaunch managed session will end.`, buttons `End session` (default, `DialogResult.OK`) and `Cancel` (`Escape`). A second request while the dialog is open activates it instead of stacking another one.
...
5. `Cancel` (or closing the dialog) does nothing: the session keeps running (`T01`).
6. A stop that arrives from elsewhere (`session stop`, Ctrl+C, `/observe-seconds`, OMSI exiting) while the status window or the confirmation dialog is open closes them as part of the indicator's disposal; the owner does not wait for the user (`T02`: owner ended 725 ms after the pipe stop with both windows open).
...
Localized strings cover the tooltip, the two menu items, the status window (heading, section titles, field labels, `Close`), the mode and presentation values, and the confirmation dialog. The glossary is maintained in `docs\windows-ui-localization.md`; offline test `windows-ui.localization-and-status` (`tests\OmsiLaunch.WindowsUiTests`) verifies resolution and the presenter.

## `OmsiLaunchW.exe` failure dialogs
...
- Creation and disposal are serialized under a lock: a disposal that wins the race makes the UI thread skip its message loop and clean up immediately.
...
- Tray messages still arrive while the confirmation dialog is modal; a second confirmation is not stacked.
...
Observed in real sessions on the authorized installation in the runtime closure round (`research/reports/runtime-closure/FINAL-RUNTIME-VALIDATION-REPORT.md`, pt-BR Windows UI; see the [runtime validation status](../status/runtime-validation-status.md)):
...
- `OmsiLaunchW.exe` failure dialogs for an argument error (`OL_E_INVALID_ARGUMENT`), no active session (`OL_E_NO_ACTIVE_SESSION`) and a session that fails before gameplay (`OL_E_WORLD_START_FAILED`) (`T04`). For the last one the dialog shows the plugin's failure payload as its message.
...
Not produced: the bootstrapper shim exit-code dialogs (`100`..`106`).
