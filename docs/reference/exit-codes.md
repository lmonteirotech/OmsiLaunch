...
| 101 | Command line could not be tokenized | `CommandLineToArgvW` returned null. |
...
| 106 | Managed host could not be initialized | `hostfxr_initialize_for_dotnet_command_line` failed for `OmsiLaunch.Controller.dll` (missing `OmsiLaunch.Controller.runtimeconfig.json`, missing `Microsoft.WindowsDesktop.App` 6.0 x64, or a damaged package). |
