...
| Arguments | tokenized with `CommandLineToArgvW` and passed unchanged to the controller | the same, so both hosts accept exactly the same flags and commands ([CLI reference](cli.md)) |
...
| Errors | `OL_E_...: message` or a JSON error envelope | the same console output rules, **plus** a message box for every error (see [Failure dialogs](#failure-dialogs)) |
...
2. It tokenizes the command line (`CommandLineToArgvW`) and sets `OMSILAUNCH_WINDOWS_HOST=1`.
3. It locates `hostfxr` through the packaged `nethost.dll`, loads it, initializes the controller with the arguments (the controller path is not part of the argument list the CLI parser sees) and runs it.
...
| `101` | the command line could not be tokenized |
...
| `106` | the managed host could not be initialized (for example `OmsiLaunch.Controller.dll` or its runtime configuration is missing, or the Windows Desktop runtime is absent) |

`OmsiLaunch.exe` uses the same table but prints nothing. These dialogs have not been produced at runtime (see [runtime validation status](../status/runtime-validation-status.md)).
...
Exit `0` means only that `OmsiLaunchW.exe` was started. The session outcome (planning errors, an owner already active, a failed start) is reported by `OmsiLaunchW.exe` with its own dialogs and diagnostics, and the two processes are independent afterwards: `OmsiLaunch.exe` has ended, and `OmsiLaunchW.exe` is the session owner. Use `OmsiLaunch.exe session status` to see the session.
...
| a launch whose plan is not runnable | dialog with the plan's last `OL_E_` diagnostic (fallback `The session plan is not runnable.` / `OL_E_SESSION_START_FAILED`), exit `1` (documentation audit BUG-06; before the fix it exited silently) |
| a launch while an owner is already active for the installation | `OL_E_SESSION_ALREADY_ACTIVE` dialog, exit `7`; the running session is not affected |
| a launch that does not reach `Running` | dialog `The OMSI session did not reach gameplay.` with the last `OL_E_` diagnostic, exit `1`. OMSI is terminated and the files are restored by the session supervisor while the dialog is open; the process exits after the dialog is closed and `CloseAsync` has finished |
| an invalid argument, unknown flag or invalid session profile | dialog with `OL_E_INVALID_ARGUMENT` or the `OL_E_SESSION_PROFILE_*` code, exit `2` |
| a client command (`session status`, `session stop`, `events read`, `time get`, `/runtime:...`) with no owner | `OL_E_NO_ACTIVE_SESSION` dialog, exit `4` |
| a client command that the owner rejects | dialog with the rejection code (for example `OL_E_CONTROL_SESSION_MISMATCH` or a runtime error code), exit `7` (`2` for an unknown operation or a missing argument) |
...
| `/plan` or `/validate` | no dialog, even for a plan that is not runnable; exit `0` or `1` |
| any other controller failure | dialog with the classified `OL_E_` code; exit code per [exit codes](exit-codes.md) |
...
`<message>` is the error message, or for a session failure the message of the last `OL_E_` diagnostic. Known limitation: for a failure reported by the plugin, the message is the plugin's raw failure payload (for example `{"name":"world.failed",...}`); the `Code:` line is correct (see [known limitations](known-limitations.md)). The dialog is modal and the process exits after it is closed. Runtime evidence: argument error, no active session and a failure before gameplay (runtime closure `T04`).
...
5. If something goes wrong, the dialog shows the error code; details are in `<OMSI_PATH>\.omsilaunch\diagnostics`.
